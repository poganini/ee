...
// date: 2012-07-11
{
    ...
    /* В params передаётся режим: announce, addform, combo, либо posts
    	Простой распространнёный режим combo;10 */
	function EGuestbook_idpo($global_params, $params, $module_id, $node_id = NULL, $module_uri = NULL, $additional = NULL)
	{
		global $Engine;
		...
		{ 
			...
			case "posts": {
				$per_page_input = (isset($parts[1]) && $parts[1]) ? $parts[1] : ""; // строка настройки для Pager
			...
			break;
                        ...
                        case "main_cat_list": {
                               ...
                               //получение олного списка категорий
                               $my_all_cat = $this->GetPostCategories();
                               
                               $this->output["module_uri"] = $Engine->unqueried_uri;
                               if (!$array = parse_ini_file(INCLUDES . $parts[2], true))
                                                    {
                                                      die("EGuestbook module error #1002 at node $this->node_id: cannot process config file '$parts[2]'.");
                                                    } 
                                
                               
                               
                               if (!isset($_GET["my_cat"]))
                                    {
                                     if (isset($_POST["are_categories"]))
                                        ...
                                        {   	
                                            $this->output["main_categories"] = $my_all_cat;
                                            $this->output["category_pictures"] = $array["category_pictures"];
                                        }
                                    }
                               elseif (is_numeric($_GET["my_cat"]))
                                    {
                                   
                                   $this->output["main_categories"][]=$_GET["my_cat"];
                                   unset($_SESSION[$this->db_prefix.'categories']);
                                   $_SESSION[$this->db_prefix.'categories'][] =  $my_all_cat[$_GET["my_cat"]];
                                   $this->output["selected_category"]["name"] = $my_all_cat[$_GET["my_cat"]];
                                   $this->output["selected_category"]["pict"] = $array["category_pictures"][$_GET["my_cat"]];
                                   $this->output["selected_category"]["num"] = $_GET["my_cat"];
                                   $_SESSION[$this->db_prefix.'categories_set'] = 1;
                                   $this->Posts($per_page_input);
					...
					case "save_post":
						if (isset($POST_DATA["id"], $POST_DATA["replace_response_stats"], $POST_DATA["post_text"], $POST_DATA["author_name"], $POST_DATA["response_text"]) && CF::IsNaturalNumeric($POST_DATA["id"]) && (!$this->allow_time_alter || isset($POST_DATA["post_year"], $POST_DATA["post_month"], $POST_DATA["post_day"], $POST_DATA["post_hours"], $POST_DATA["post_minutes"], $POST_DATA["post_seconds"], $POST_DATA["response_year"], $POST_DATA["response_month"], $POST_DATA["response_day"], $POST_DATA["response_hours"], $POST_DATA["response_minutes"], $POST_DATA["response_seconds"]))) {
							...
							if (!CF::IsNonEmptyStr($POST_DATA["author_name"])) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 301; // пустое имя отправителя
							}
							...
							if (!CF::IsNonEmptyStr($POST_DATA["post_text"])) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 302; // не задан текст сообщения
							}
                                                        
                                                        if (!CF::IsNonEmptyStr($POST_DATA["post_header"])) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 302; // не задан текст сообщения
							}
							...
							if ($this->status) {
								...
								foreach ($this->additional_fields as $key => $data) {
									if (isset($POST_DATA[$data["subkey"]])) {
										$DB->AddValue($key, $POST_DATA[$data["subkey"]]);
                                                                ...
                                                                $DB->AddValue("expert_responder_id", $POST_DATA["expert_responder_id"]); 
                                                                
                                                                
								if (CF::IsNonEmptyStr($POST_DATA["response_text"])) {
									if ($POST_DATA["replace_response_stats"]) { // если ещё не было ответа, а теперь он поступил, записываем текущего пользователя и время
										if (!$this->allow_time_alter) {
								...
								} else { //if (!$POST_DATA["replace_response_stats"]) // если текст пустой, но перед этим не был пустым   !!! подумать над более правильной формулировкой условия
									$DB->AddValue("response_time", 0);
									$DB->AddValue("respondent_id", 0); // id пользователя, удалившего ответ
									$DB->AddValue("unrespondent_id", $Auth->user_id); // id пользователя, удалившего ответ
								}
								...
								if (is_string($DB->Update(1))) {
									$status = false;
									$this->output["messages"]["bad"][] = 402; // ошибка БД при сохранении сообщения и ответа
								} else {
									...
									if ($this->notify_settings["enable_to_author"]) { //  && $value)									
										...
										{
											$array = array(		// Массив подстановок перменных с процентами из инишника
							...
							if (!$this->status)
							{
								...
								$this->form_data = array(
									...
									"replace_response_stats" => $POST_DATA["replace_response_stats"],
                                                                        "post_header" => htmlspecialchars($POST_DATA["post_header"]),
		...
		elseif (isset($_GET["node"], $_GET["action"]) && ($_GET["node"] == $this->node_id))
		{
			switch ($_GET["action"])
			{
				case "edit_post":
					...
					{
						$DB->SetTable($this->db_prefix . "guestbook");
						$DB->AddFields(array("post_text", "post_category", "author_name", "response_time", "response_text", "respondent_id","expert_responder_id", "post_header"));
						...
						{ 
							...
							$this->form_data = array(
								...
								"post_text" => htmlspecialchars($row->post_text),
                                                                "post_header" => htmlspecialchars($row->post_header),
								...
								"categories" => $this->GetPostCategories(),
                                                                "experts"=> $this->GetExpertList(),
                                                                "expert_responder_id" => htmlspecialchars($row->expert_responder_id)
                                                                );
				...
				case "unhide_post":
					if (isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
						...
						{
							$this->status = false;
							$this->output["messages"]["bad"][] = $value ? 404 : 403; // ошибка БД при скрытии/открытии сообщения
						}

						else
						{
							$this->status = true;
							$this->output["messages"]["good"][] = $value ? 104 : 103; // сообщенияе успешно скрыто/открыто
							// сюда добавим !!!!
				...
				case "delete_post":
					...
					if (isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
						if (is_string($DB->Exec("DELETE FROM `" . $this->db_prefix . "guestbook`
							...
							LIMIT 1")))
						{
							$this->output["messages"]["bad"][] = 305; // ошибка БД при удалении сообщения
						}
	...
	function Announce($module_folder_id, $display_limit)
	{
		...
		$DB->SetTable($this->db_prefix . "guestbook");
		$DB->AddFields(array("id", "post_time", "post_text", "post_category", "author_id", "author_name", "author_from", "author_phone", "author_email", "response_time", "response_text", "respondent_id","post_header"));
       ...
       function GetExpertList() 
	{
		...
		$DB->SetTable($this->db_prefix . "people");
                $DB->AddFields(array("id", "user_id", "last_name", "name", "patronymic", "photo", "comment", "prad", "email", "expert_in", "phone"));
	...
	function AddForm()
	{
		...
		if (isset($_POST[$this->node_id]["name"], $_POST[$this->node_id]["post"]) && (!$this->captcha_settings["enable"] || isset($_POST[$this->node_id]["code"])))
		{
			...
			{
				$this->status = false;
				$this->output["messages"]["bad"][] = 301; // пустое имя
			}
			...
			foreach ($this->additional_fields as $data)
			{
				if (!isset($_POST[$this->node_id][$data["subkey"]])) // Если не определены в POST дополнительные поля
				{
					$this->status = false;		// То, без всякого вывода ошибок, облом :) 
					break;
				}
			...
			{				
				...
				$DB->AddValue("post_text", $_POST[$this->node_id]["post"]);
                                $DB->AddValue("post_header", $_POST[$this->node_id]["post_header"]);
	...
	function Posts($per_page_input)
	{
		...
		"response_time", "response_text", "respondent_id", "expert_responder_id","post_header"));
		...
		if ($this->auth_handle_mode != "ignore")
		{
			...
			$DB->AddFields(array(AUTH_DB_PREFIX . "users.displayed_name", AUTH_DB_PREFIX . "users.email"));
                	...
                	//Приcтыковка данных эксперта из таблицы idpo_people
                        $DB->AddJoin($this->db_prefix . "people.id", "expert_responder_id");
			$DB->AddFields(array( 
                                             $this->db_prefix . "people.last_name", 
                                             $this->db_prefix . "people.name", 
                                             $this->db_prefix. "people.patronymic", 
                                             $this->db_prefix. "people.photo", 
                                             $this->db_prefix. "people.comment", 
                                             $this->db_prefix. "people.prad", 
                                             $this->db_prefix. "people.email", 
                                             $this->db_prefix. "people.expert_in", 
                                             $this->db_prefix. "people.phone"));
		...
		while ($row = $DB->FetchObject($res))
		{
			$this->output["posts"][] = array(
				...
				"post_time" => $row->post_time,
				"post_text" => $this->break_lines ? nl2br($row->post_text) : $row->post_text,
                                "post_header" => $this->break_lines ? nl2br($row->post_header) : $row->post_header,
				"post_category" => $row->post_category,
				"author_id" => $row->author_id,
				...
				"response_text" => $this->break_lines ? nl2br($row->response_text) : $row->response_text,
				"respondent_id" => $row->respondent_id,
                                "expert_responder_id" => $row->expert_responder_id,
                                "expert_last_name" => $row->last_name,
                                "expert_name" => $row->name,
                                "expert_patronymic" => $row->patronymic,
                                "expert_photo" => $row->photo,
                                "expert_prad" => $row->prad,
                                "expert_expert_in" => $row->expert_in,
                                "expert_email" => $row->email,
                                "expert_phone" => $row->phone
