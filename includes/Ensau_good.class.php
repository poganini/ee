    ...
    function timetable_teachers($mode, $dep_id)
    {
        ...
        switch($mode[0])
        {
            case "edit":
            {
                if(!isset($_POST["id"]))
                {
                    ...
                    if($row = $DB->FetchAssoc($res))
                    {
                        ...
                        $res2 = $DB->Select();
                        if($row2 = $DB->FetchAssoc($res2))
                            $row["department1_id"] = $row2["department_id"];
                        else
                            $row["department1_id"] = 0;
                        if($row2 = $DB->FetchAssoc($res2))
                            $row["department2_id"] = $row2["department_id"];
                        else
                            $row["department2_id"] = 0;
                        if($row["user_id"])
                        {
                            $DB->SetTable("auth_users");
                            $DB->AddCondFS("id","=",$row["user_id"]);
                ...
                if(isset($_POST["last_name"]))
                {
                    if(!isset($_POST["check"]))
                    {
                        ...
                        if($row = $DB->FetchAssoc($res)) {							
							...
							$res1 = $DB->Select();
							while($row1 = $DB->FetchAssoc($res1)) {
								if($row1["department_id"] == $_POST["department1_id"] || $row1["department_id"] == $_POST["department2_id"]) {
            ...
            //CF::Debug($Auth);
				...
				$DB->AddField("d.name","name");
				$DB->AddField("d.id","id");
				$DB->AddAltFF("d.faculty_id","=","f.id");
				$res =  $DB->Select();
				while($row = $DB->FetchAssoc($res))
				{
					if ($Engine->OperationAllowed(5, "teachers.faculty.handle", $row["faculty_id"], $Auth->usergroup_id) || $Engine->OperationAllowed(5, "teachers.own.handle", $row["id"], $Auth->usergroup_id))
						$this->output["department"][] = $row;
						//die($row["faculty_id"]);
				}
				break;
