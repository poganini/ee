...
/*		// Обработка файла параметров
		...
		foreach ($this->config["form_elems"] as $key => $data) { 				// Проходим по полям формы, описанным в конфигурации,...
			if ($data["type"] != "f") {                       					// ...и если тип поля - не "файл",...			
				$this->form_data[$key] = "";                   					// ...то изначально задаём пустое значение для поля формы,...
				$maxlen[$key] = isset($data["conds"]["maxlen"]) ? $data["conds"]["maxlen"]["value"] : false;	// ...а также сохраняем максимальную длину для поля формы (или false, если она не указана).
			}
		}

		if ($this->config["antispam_settings"]["enable_hhf"]) { 				// При включенном алгоритме HHF...		
			$key = $this->config["antispam_settings"]["hhf_elem_name"];
			$this->form_data[$key] = "";										// ...также дописываем изначально пустое значение в его поле...
			$maxlen[$key] = $this->config["antispam_settings"]["hhf_elem_maxlen"]; 		// ...и его maxlen.
		}
		...
		$this->output["having_files"] = $this->config["general_settings"]["having_files"]; 		// ...то, есть ли в форме файлы,...
		$this->output["max_filesize"] = $this->config["general_settings"]["max_filesize"]; 		 // ...и какой максимальный размер файла.
