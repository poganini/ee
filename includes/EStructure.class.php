...
// date: 2013-11-22
{
	...
	// Облегченная версия функции FolderList для вывода выпадающего списка папок
							...
							if($res = $DB->Select()) {
								if($row = $DB->FetchObject($res)) {
									...
									if (!$Engine->OperationAllowed($row->id, "item.alter", $texter_id, $Auth->usergroup_id))
										$Engine->AddPrivilege($row->id, "item.alter", $texter_id, $Auth->usergroup_id, 1, "create; создание текстера, добавление права на его редактирование");
