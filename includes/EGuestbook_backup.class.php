...
// date: 2010-04-02
{
		...
		elseif (isset($_GET["node"], $_GET["action"]) && ($_GET["node"] == $this->node_id))
		{
			switch ($_GET["action"])
			{
				case "edit_post":
					...
					{
						$DB->SetTable($this->db_prefix . "guestbook");
						$DB->AddFields(array("post_text", "author_name", "response_time", "response_text", "respondent_id"));
	...
	function Announce($module_folder_id, $display_limit)
	{
		...
		$DB->SetTable($this->db_prefix . "guestbook");
		$DB->AddFields(array("id", "post_time", "post_text", "author_id", "author_name", "author_from", "author_phone", "author_email", "response_time", "response_text", "respondent_id"));
			...
			{				
				...
				else
				{
					...
					if ($this->notify_settings["enable"])
					{
						...
						$post = str_replace(array_keys($array), array_values($array), $this->notify_settings);
						mail($post["to"], $post["subject"],	$post["message"], $post["headers"]);
					}
	...
	function Posts($per_page_input)
	{
		...
		$DB->SetTable($this->db_prefix . "guestbook");
		$DB->AddFields(array("id", "is_active", "post_time", "post_text", "author_id", "author_name", "author_from", "author_phone", "author_email", 
		"response_time", "response_text", "respondent_id"));
