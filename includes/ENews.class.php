...
// date: 2014-04-10
{
	...
	// В $global_params (Поле engine_modules.global_params) передаётся конфигурационный файл
	// В $params (Поле engine_nodes.params) передаётся через точку с запятой режим (announce, full_list), номер категории (0, 1, 2 ...)
	...
	function ENews($global_params, $params, $module_id, $node_id = NULL, $module_uri = NULL, $additional = NULL)
	{
		...
		$this->output["manage_access"] = $node_id && $Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id); // !!! избавиться от пережитков

//		$this->output["manage_access"] = $Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id);  // !!! неправильно, т.к. изначально current_cat_id равно NULL
		...
		$this->use_tags = $array["general_settings"]["use_tags"]; // Использовать метки
...
//		}
		...
		switch ($this->mode) {			
			...
			case "header_news": {
				$this->output["news_text"] = "Начало занятий с 3 сентября согласно расписанию";
			}
			break;
			...
			case "full_list": { 
				...
				$per_page_input = (isset($parts[2]) && $parts[2]) ? $parts[2] : ""; // строка настройки для Pager
				$include_subcats = (isset($parts[3]) && $parts[3]); // также включать новости подкатегорий
				$tag_id = (isset($parts[4]) && $parts[4]) ? $parts[4] : ""; // id метки, если есть
				...
				$this->output["cat_options"] = $this->FolderCatOptions(); // !!! подумать
			...
			case "subscribe": {
				...
				if (isset($_GET["code"])) {
					if ($this->ConfirmSubscriber($_GET["code"])) {
						$this->output["messages"]["confirmation_result"] = "<p class=\"message\">Подписка подтверждена!</p>\n";
					}
					else {
						$this->output["messages"]["confirmation_result"] = "<p class=\"message red\">Передан неверный код подтверждения!</p>\n";
					}
				}
          ...
          if(!CF::ValidateEmail($_POST[$this->node_id]["subscribe"]["email"])) {
            ...
            $this->output["messages"]["bad"][] = 312;
					...
					if ($array["subscribes_settings"]["need_confirmation"]) {
						...
						/*$headers .= "To: ".$name." <".$email"].">\r\n";*/ // Не будем, потому что не хочется вносить в цикл while всё это формирование
						...
						$subject = SITE_SHORT_NAME."Подтверждение подписки";
						
						$message = "Здравствуйте! Чтобы подтвердить подписку на рассылку новостей сайта \"".SITE_SHORT_NAME."\", перейдите по ссылке: http://".$_SERVER["SERVER_NAME"]."/".$array["subscribes_settings"]["confirm_uri"]."?code=".$confirm;
					...
					if (!$DB->Insert()) {
						$status = false;
						$this->output["messages"]["bad"][] = 401; // ошибка БД при добавлении материала
					}
					else {
...
//				"author_name" => $this->show_users ? $this->displayed_name : "",
	...
	function RSS($folder_id, $display_limit, $include_subcats) {
        global $DB, $Engine;
        $this->output["folder_uri"] = $folder_uri = $Engine->FolderURIbyID($folder_id); // сделать просчёт нужной папки
...
//        }
        ...
        if($this->mode != "new_full_list") {
			$DB->AddCondFX("time", "<=", "NOW()"); // сделать опцией?
		}
					...
					case "add_item": {
						if (($Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id) || $Engine->ModuleOperationAllowed("cat.create.items", $this->current_cat_id)) && isset($POST_DATA["cat_id"], $POST_DATA["uripart"]/*, $POST_DATA["public"]["year"], $POST_DATA["public"]["month"], $POST_DATA["public"]["day"], $POST_DATA["public"]["hours"], $POST_DATA["public"]["minutes"], $POST_DATA["seconds"]*/, $POST_DATA["title"], /*$POST_DATA["image_ext"], */$POST_DATA["short_text"], $POST_DATA["full_text"]) && CF::IsNaturalOrZeroNumeric($POST_DATA["cat_id"])) {
							...
							if (strlen($POST_DATA["title"]) < 3) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 912; // не указана заголовок!)
							}
							...
							if (CF::IsNonEmptyStr($POST_DATA["uripart"]) && (preg_match("/^\d+$/", $POST_DATA["uripart"]) || !preg_match("/^[a-z0-9_-]+$/i", $POST_DATA["uripart"]))) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 302; // недопустимый формат uripart (только латинские буквы, цифры, дефис и подчерк; строка, состоящая целиком из&nbsp;цифр, недопустима!)
							}
							...
							if ($this->mode == 'new_full_list' && (!isset($POST_DATA["start_event"]["date"]) || !$POST_DATA["start_event"]["date"])) {
								$this->status = false;
								$this->output["messages"]["bad"][] = 304; // не задан краткий текст
							}
							...
							if ($this->status) {
								...
								if (!$DB->Insert()) {
									...
									$this->output["messages"]["good"][] = 101; // материал успешно добавлен
									$ITEM_ID = $DB->LastInsertID();
									
									$this->SendToSubscribers($ITEM_ID);
									
									if($this->use_tags && $POST_DATA["tags"]) {
									 	$tags = explode(",", $POST_DATA["tags"]); // разбиваем по запятой
									 	foreach ($tags as $tag) {
									 		$tag = trim($tag);
						...
						if (($Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id) || $Engine->ModuleOperationAllowed("cat.create.items", $this->current_cat_id)) && isset($POST_DATA["id"], $POST_DATA["cat_id"], $POST_DATA["uripart"]/*, $POST_DATA["public"]["year"], $POST_DATA["public"]["month"], $POST_DATA["public"]["day"], $POST_DATA["public"]["hours"], $POST_DATA["public"]["minutes"], $POST_DATA["seconds"]*/, $POST_DATA["title"], /*$POST_DATA["image_ext"], */$POST_DATA["short_text"], $POST_DATA["full_text"]) && CF::IsNaturalNumeric($POST_DATA["id"]) && CF::IsNaturalOrZeroNumeric($POST_DATA["cat_id"]))
						{
							...
							if ($this->status)
							{
								...
								else
								{
									...
									if($this->use_tags)
									{
										// Удалим все связи меток и новостей
										$DB->SetTable("tags_posts");
										...
										if ($POST_DATA["tags"]) {
											...
											foreach ($tags as $tag)
											{
												...
												if($row = $DB->FetchObject($res)) // Если такая метка уже есть
												{
													...
													$DB->AddValue("link_suffix", $Engine->engine_uri.$POST_DATA["id"].".".$this->item_ext); // !!!! Очень временно!
													$DB->AddValue("cache_title", $POST_DATA["title"]);
													$DB->AddValue("cache_short", $POST_DATA["short_text"]); 
													$DB->Insert();									 			
													// echo "<strong>".$row->id."</strong><br />";
					...
					case "add_comment":
					{
						if ($Engine->ModuleOperationAllowed("cat.comment", $this->current_cat_id) && isset(/*$POST_DATA["item_id"], */$POST_DATA["author_name"], $POST_DATA["author_from"], $POST_DATA["author_email"], $POST_DATA["text"]) && CF::IsNaturalNumeric($POST_DATA["item_id"]))
						{
							...
							if ($this->status)
							{
								...
								$DB->AddValue("item_id", $POST_DATA["item_id"]);  //  !!! $this->current_item_id // item_id больше не передаётся из формы, в итоге комментарий можно добавлять только к текущему материалу, зато ID материала подделать нельзя
			...
			switch ($_GET["action"])
			{
				...
				case "edit_item": {
					if (($Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id) || $Engine->ModuleOperationAllowed("cat.create.items", $this->current_cat_id)) && isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"])) {
						...
						if ($this->use_tags)	// Если используем метки
						{
						...
						else
						{
							$DB->FreeRes($res);
							$this->status = false;
							$this->output["messages"]["bad"][] = 310; // материал не найден в базе данных
						}
					}
				}
				break;
				...
				case "unhide_item": {
					if (($Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id) || $Engine->ModuleOperationAllowed("cat.create.items", $this->current_cat_id)) && isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
						...
						{
							$this->status = true;
							$this->output["messages"]["good"][] = $value ? 104 : 103; // новость успешно скрыта/открыта
							$Engine->LogAction($this->module_id, "item", $_GET["id"], $value ? "hide" : "unhide");
						}
						else
						{
							$this->status = false;
							$this->output["messages"]["bad"][] = $value ? 404 : 403; // ошибка БД при скрытии/открытии новости
						}
					}
				}
				break;
					...
					if ($Engine->ModuleOperationAllowed("cat.items.handle", $this->current_cat_id) && isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
						...
						if ($row = $DB->FetchObject($res))
						{
							...
							if ($DB->Exec("
								...
								"))
							{
								$this->output["messages"]["good"][] = 105; // новость успешно удалена
...
//									LIMIT 1");
							...
							{
								$this->output["messages"]["bad"][] = 405; // ошибка БД при удалении новости
							}
						}
				...
				case "edit_comment": {
					if ($Engine->ModuleOperationAllowed("cat.comments.handle", $this->current_cat_id) && isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
						...
						else
						{
							$DB->FreeRes($res);
							$this->status = false;
							$this->output["messages"]["bad"][] = 318; // комментарий не найден в базе данных
						}
					}
				}
				break;
				...
				case "unhide_comment": { 
					if ($Engine->ModuleOperationAllowed("cat.comments.handle", $this->current_cat_id) && isset($_GET["id"]) && CF::IsNaturalNumeric($_GET["id"]))
					{
							...
							"))
						{
							...
							{
								$this->status = true;
								$this->output["messages"]["good"][] = $value ? 109 : 108; // комментарий успешно скрыт/открыт
						...
						{
							$this->status = false;
							$this->output["messages"]["bad"][] = $value ? 409 : 408; // ошибка БД при скрытии/открытии комментария
						}
...
//							$DB->FreeRes($res);
								...
								"))
							{
								if ($DB->AffectedRows())
								{
									$this->output["messages"]["good"][] = 110; // комментарий успешно удалён
							...
							{
								$this->output["messages"]["bad"][] = 410; // ошибка БД при удалении комментария
							}
//						}
...
//						}
	...
	function Announce($folder_id, $display_limit, $include_subcats, $skip = NULL, $sort_params = false, $sort_orders = false)
	{
		...
		//считаем общее количество для данной категории		
		$DB->SetTable($this->db_prefix . "items");
		$DB->AddField("id");
		$DB->AddAltFS("cat_id", "=", $this->current_cat_id);
...
//				"author_name" => $this->show_users ? $this->displayed_name : "",
	...
	function DateToCyr($date) {
		$monthes = array('January' => 'Январь', 'February' => 'Февраль', 'March' => 'Март', 'April' => 'Апрель', 'May' => 'Май', 'June' => 'Июнь', 'July' => 'Июль', 'August' => 'Август', 'September' => 'Сентябрь', 'October' => 'Октябрь', 'November' => 'Ноябрь', 'December' => 'Декабрь');
...
//		{
			...
			{ 
				...
				{
					$is_item = (substr($uripart, -(strlen($this->item_ext) + 1)) == ".$this->item_ext"); // проверяем, имеет ли часть пути необходимое расширение
...
//						{
							...
							if ($ENTRY_KEY === "")
							{
								continue; // избавляемся от /category/.html ;)
							}
//						}
...
/*						else
						{
							...
							$this->output["messages"]["bad"][] = 301; // новость не найдена
						}*/
...
/*		$this->current_uri = $Engine->engine_uri;
		...
		// !!! сделать нормальную проверку URI
...
//		$this->output["footsteps"] = $this->footsteps;
	...
	function ListComments($item_id, $cat_id, $limit = false)
	{
		...
		if ( (CF::IsNaturalNumeric($item_id) || $item_id==-1) && $Engine->ModuleOperationAllowed("cat.comment", $cat_id)) // !!! проверяется раньше, нехорошо что проверяем заново
		{
			$DB->SetTable($this->db_prefix . "comments");
			...
			$DB->AddOrder("time", true); // !!! По хорошему вынести в настройки: сверху либо снизу новые комментарии
	...
	{ 
		...
		$this->output["news"] = array();
		$this->output["folder_uri"] = $folder_uri = $Engine->engine_uri; // !!! некрасиво
...
//			}
			...
			while ($row = $DB->FetchObject($res)) {
				...
				if ($tag_id) //если выбираем новости по метке, то адрес будет взят из таблицы меток
					$link = $row->link_suffix;
				else //иначе из таблицы новостей
...
//					"author_name" => $this->show_users ? $this->displayed_name : "",
	...
	function CatURIbyID($id, $folder_id)
	{
		...
		while ($id != $this->root_cat_id)
		{
			...
			{
				$DB->FreeRes($res);
				if (CF::IsNonEmptyStr($row->uripart)) // Строчка от poganini ;-)
					$footsteps[$id] = $row->uripart; // Дабы не было ссылок типа www.example.com/news//1.html
				$id = $row->pid;
			}
	...
	function ListCats($pid = 0, $flat = false)
	{
		...
		while ($row = $DB->FetchObject($res))
		{
			...
			{
				$output[$row->id] = array(
					"uripart" => $row->uripart, // возможно, надо выдавать полный путь
					"title" => $row->title,
					"descr" => $row->descr,
	...
	/* Функции по комментированию */

	/* Функции по подписке */

	function SendToSubscribers($news_id) // Отправить новость подписчикам
	{
		global $DB;
		...
		$count = 0; // Число удачных посылок писем
		// Начинаем рассылку
		while ($row = $DB->FetchObject($res))
		{
	...
	function ClearSubscribers() // Удаление неподтверждённых подписчиков
	{
		global $DB;
		...
		$res = $DB->Exec($query);
		$count = $DB->AffectedRows($res); // Число удалённых не подтвердившихся подписчиков
		$DB->FreeRes($res);
		return $count;
	}
	...
	function AddSubscriber($email, $cats) // Добавляем подписчика, передаём е-мэйл и массив категорий новостей
	{
		global $DB;
		// Добавляем е-мэйл, если такого нет, а если есть, то игнорируем добавление е-мэйла
		$query = "INSERT IGNORE INTO ".$this->db_prefix."subscribers (`email`, `register_time`) VALUES (".$DB->Escape($email).", NOW())";
		$res = $DB->Exec($query);
		$DB->FreeRes($res);
		// Добавляем категории подписчику, для повторяющихся игнорим добавку
		$query = "INSERT IGNORE INTO ".$this->db_prefix."subscribe_cats (`subscriber_id`, `cat_id`) VALUES ";
		$query_values = "";
		$first_value = true; // Просто флаг первое ли значение, нужно для неуказания запятой
		foreach ($cats as $i => $cat_id) // Добавим каждую категорию в таблицу
		{
			if(!$first_value)
				$query_values .= ", ";
			else
	...
	function ConfirmSubscriber($code) // Подтверждение подписчиков    CONCAT(s.id, s.email)
	{
		global $DB;
		...
		$res = $DB->Exec($query);
		$count = $DB->AffectedRows($res); // Число подтверждённых подписчиков, по-хорошему должен быть один
		$DB->FreeRes($res);
		return $count;
	}


	function DeleteSubscriber($code)  // Удаление подписчиков CONCAT(s.id, s.email)
	{
		global $DB;
		...
		AND
			`code` = '".$DB->Escape($code)."'";
		$res = $DB->Exec($query);
		$count = $DB->AffectedRows($res); // Число удалённых подписчиков, по-хорошему должен быть один
		$DB->FreeRes($res);
		return $count;
	}


	function GetCode($email) // Получение кода
	{
		global $DB;
